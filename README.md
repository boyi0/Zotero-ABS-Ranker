...
Built specifically for Zotero 7, it utilizes an embedded offline database, in-memory caching, and bulk database transactions for lightning-fast performance.
...
- **Update All Items:** New in v1.1.0 — right-click and choose **"Update ABS ranking — All Items"** to scan and tag your entire library in one click.
- **Cached Rankings Database:** The journal database is loaded once on startup and cached in memory, making repeated lookups instant with zero disk overhead.
...
- **Auto-Update Support:** The plugin checks for updates from GitHub automatically so you always have the latest rankings and features.
...
### 📦 Installation
1. Download the latest `.xpi` file by clicking this direct link: **[⏬ Download zotero-abs-ranker-v1.1.0.xpi](https://github.com/boyi0/Zotero-ABS-Ranker/raw/main/zotero-abs-ranker-v1.1.0.xpi)**.
...
插件内嵌了完整的本地离线数据库，采用内存缓存与批量数据库事务写入技术，无需联网即可体验极速打标。
...
- **一键全库更新:** v1.1.0 新增 — 右键选择 **"Update ABS ranking — All Items"** 即可一键扫描并标注整个文献库中的所有文献。
- **智能缓存:** 期刊数据库在启动时一次性加载并缓存至内存，后续查找零磁盘开销，瞬时响应。
...
- **自动更新:** 插件会自动从 GitHub 检查版本更新，确保你始终使用最新的期刊数据和功能。
- **操作完成反馈:** 每次运行结束后，会弹出摘要提示框，告知本次成功更新了几条文献、有几条未能在数据库中找到匹配期刊。

### 📦 如何安装
1. 点击这里直接下载全功能插件安装包： **[⏬ 下载 zotero-abs-ranker-v1.1.0.xpi](https://github.com/boyi0/Zotero-ABS-Ranker/raw/main/zotero-abs-ranker-v1.1.0.xpi)** 。
